...
indir_plot <- indir_raw %>%
  ...
  mutate(
    ...
    scenario = recode(scenario,
                      "implemented" = "UNICEF response",
                      "30"          = "30% coverage",
                      "95"          = "95% coverage"),
    scenario = factor(scenario, levels = c("UNICEF response","30% coverage","95% coverage")),
...
p <- ggplot(indir_plot, aes(x = emergency, y = musd, fill = scenario)) +
  ...
  geom_text(aes(label = number(musd, accuracy = 0.1)),
  ...
  labs(
    x = NULL, y = "Millions of 2015 USD", fill = NULL,
    title = "Estimated Indirect Economic Benefits from NiE by Scenario",
    subtitle = "Sum of avoided formula expenditure and preserved future income via EBF (undiscounted)",
    caption = "Source: Authors’ analysis of UNICEF NiE data (2015 USD)."
  ) +
...
ggsave("figures/econ-benefits-bar-chart.png", p, width = 8, height = 5, dpi = 300)

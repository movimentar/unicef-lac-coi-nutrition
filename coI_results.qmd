...
tar_read(coi_dir_benefits)$breastfeeding %>%
  ...
  gt(groupname_col = "emergency") %>%
  tab_header(title = "Estimated improvements in breastfeeding indicators by nutrition responce scenario per emergency using the LiST model") %>%
  cols_hide("indicator_category") %>%
  fmt_number(columns = everything(),
             decimals = 0) %>%
  cols_align("center") %>%
...
tar_read(coi_indir_benefits) %>% 
  ...
  tab_header(title = "
Estimated indirect benefits by nutrition responce scenario per emergency from exclusive breastfeeding improvements") %>% 

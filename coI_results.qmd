...
```{r}
#| label: econ-benefits-bar-chart
#| fig-cap: "Estimated Indirect Economic Benefits from Full NiE Coverage in Two Emergency Settings (in millions USD)"
#| fig-alt: "Grouped bar chart showing undiscounted indirect economic benefits (millions of 2015 USD) for Eta-Iota and Migration flows, across scenarios: UNICEF response, 30% coverage, 95% coverage."
#| warning: false
#| message: false
...
indir_raw <- tar_read(coi_indir_benefits)

indir_plot <- indir_raw %>%
  ungroup() %>%
  filter(indicator_name_absolute %in% c(
    "Cost of feeding a child with formula for the first 2 years",
    "Potential future income lost due to cognitive losses",
    "Potential future income lost due to cognitive losses (undiscounted)"
  )) %>%
  filter(!grepl("\\bPV\\b", indicator_name_absolute)) %>%
  mutate(
    across(contains("Eta-Iota"), abs),
    across(contains("Migration"), abs)
  ) %>%
  pivot_longer(
    cols = matches("Eta-Iota|Migration\\sflows"),
    names_to = c("emergency", "scenario"),
    names_sep = "_",
    values_to = "usd"
  ) %>%
  group_by(emergency, scenario) %>%
  summarise(usd = sum(usd, na.rm = TRUE), .groups = "drop") %>%
  mutate(
    musd = usd / 1e6,
    scenario = recode(scenario,
      "implemented" = "Current UNICEF Response",
      "30"        = "30% Coverage",
      "95"        = "95% Coverage"),
    scenario = factor(scenario, levels = c("Current UNICEF Response", "30% Coverage", "95% Coverage")),
    emergency = factor(emergency, levels = c("Eta-Iota", "Migration flows"))
  )

p <- ggplot(indir_plot, aes(x = emergency, y = musd, fill = scenario)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = number(musd, accuracy = 1)),
  ...
  labs(
    x = NULL, y = "Millions of USD (2015, Undiscounted)", fill = "Scenario",
    title = "Hundreds of Millions in Benefits Forgone: The Economic Cost of Underfunding NiE",
    subtitle = "The economic benefits of interventions span two periods: avoided formula costs during\na child's first two years (0–24 months), and increased lifetime earnings projected from 2038–2080.",
    caption = "Source: UNICEF CoI Study. Indirect benefits include avoided formula costs and increased lifetime earnings."
  ) +
  scale_fill_manual(values = c("#A6CEE3", "#1F78B4", "#029ee2")) +
...
# ggsave("figures/econ-benefits-bar-chart.png", p, width = 6, height = 5, dpi = 300)
```

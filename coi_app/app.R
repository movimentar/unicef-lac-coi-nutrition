...
fmt_dollar <- function(x) as_chr(scales::dollar(round(x, 0)))
...
# ---------- Gemini (REST; X-Goog-Api-Key header) ----------
...
# Safely extract a human-readable message from try/condition/character
...
gemini_generate_rest_retry <- function(..., max_retries = 5L, base_delay = 1.5){
  ...
  repeat {
    ...
    # OK if not a try-error AND doesn't look like a failure string
    ...
    # Only retry on rate-limit/server-side transient errors
    if (!grepl("429|Too Many Requests|Resource exhausted|503|temporar", msg, ignore.case = TRUE) ||
        i >= max_retries) {
...
# ---------- Narrative prompt (includes top indicators) ----------
build_coi_prompt <- function(p, indicator_lines = NULL){
  sprintf(paste(
    ...
    if (length(indicator_lines)) "\nKey direct indicators (planning period):\n" else "",
...
plw_aliases <- c(
  ...
  "Pregnant & lactating women", "Pregnant and lactating women"
)

pin_baseline <- coi_costs %>%
  mutate(
    tg_norm = tolower(gsub("\\s+", " ",
                           chartr("–—", "--", trimws(target_group))))
  ) %>%
  group_by(emergency) %>%
  summarise(
    base_total = sum(ideal_delivered, na.rm = TRUE),
    baseline_children_raw = sum(ideal_delivered[
      tg_norm %in% tolower(gsub("\\s+", " ", children_aliases))
    ], na.rm = TRUE),
    baseline_plw_raw = sum(ideal_delivered[
      tg_norm %in% tolower(gsub("\\s+", " ", plw_aliases))
    ], na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    baseline_children = dplyr::coalesce(na_if(baseline_children_raw, 0),
                                        ifelse(base_total > 0, base_total * 0.6, 1)),
    baseline_plw      = dplyr::coalesce(na_if(baseline_plw_raw, 0),
                                        ifelse(base_total > 0, base_total * 0.4, 1))
  ) %>%
  select(emergency, baseline_children, baseline_plw)

# ---------- Direct benefits anchors (clean per emergency) ----------
build_direct_anchors <- function(maln_tbl, bf_tbl, emergency){
  maln <- maln_tbl %>%
...
ui <- page_fluid(
  ...
  div(class = "logo-container",
       # tags$img(src = "united-nations-childrens-fund-unicef-vector-logo.png", height = "40"), # UNICEF logo (disabled since in mockup)
      ...
      accordion(
        ...
        accordion_panel("Advanced Settings",
                        ...
                        tooltip(
                          ...
                          "Duration of the underlying study. Used to annualise costs/benefits before scaling."
                        ),
                        ...
                        conditionalPanel("input.override_costs",
                                         ...
                                         helpText("Apply a proportional change to total costs (e.g., local prices/logistics).")
                        )
        )
      ),
      div(class = "footer",
...
server <- function(input, output, session){
  ...
  # Direct anchors (cleaned) for selected emergency
  ...
  direct_by_indicator <- reactive({
    ...
    y30 <- as.numeric(da$`30`)
    y95 <- as.numeric(da$`95`)
    y0  <- 0
    
    y_cov <- if (cov <= 30) {
      y0 + (y30 - y0) * (cov / 30)
    } else {
      y30 + (y95 - y30) * ((cov - 30) / (95 - 30))
    }
    
    study_total <- as.numeric(y_cov) * as.numeric(scale_vec)
    
    yrs_study <- as.numeric(input$study_years); if (!is.finite(yrs_study) || yrs_study<=0) yrs_study <- 3
    yrs_plan  <- as.numeric(input$years);       if (!is.finite(yrs_plan)  || yrs_plan <=0) yrs_plan  <- 1
  ...
  results <- reactive({
    ...
    indir_total <- annualise(indir_total_study, input$study_years) * input$years
    cost_total  <- annualise(cost_total_study,  input$study_years) * input$years
  ...
  output$benefit_plot <- renderPlot({
    ...
    ggplot(df, aes(metric, usd, fill = metric)) +
      ...
      scale_fill_manual(values = c("Indirect benefits" = UNICEF_CYAN,
                                   "Cost"               = "#6C757D")) +
  ...
  # ---------- AI Interpretation (button-triggered) ----------
  ...
  observeEvent(input$generate_narrative, {
    ...
    # If the retry helper returned an error-like string, surface it nicely
  ...
  #   content=function(file) readr::write_csv(summary_df_export(), file)
  # )
  ...
  # ---------- HTML Report (opens in new tab with progress bar) ----------
  observeEvent(input$generate_report, {
    ...
    withProgress(message = "Rendering HTML report…", value = 0, {
      ...
      # create a per-session resource path backed by a temp dir
